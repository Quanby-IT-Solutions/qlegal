...
// import { CheckCircle, Clock, Download, Eye, FileText, MoreVertical, User } from "lucide-react"
...
// function getSignatureProgress(doc: { recipients?: Array<{ status: string; role?: string }> }) {
...
// 	const percent = totalCount === 0 ? 0 : Math.round((signedCount / totalCount) * 100)

// 	return { signed: signedCount, total: totalCount, percent }
// }

// export function SignedDocumentCard({ envelope, viewMode }: SignedDocumentCardProps) {
...
// 	if (viewMode === "list") {
// 		return (
// 			<Card className="group border-border bg-background hover:border-foreground/20 dark:bg-muted/60 cursor-pointer overflow-hidden border transition-all duration-200 hover:shadow-sm">
...
// 										<div className={`h-2 w-2 rounded-full ${getStatusColor(envelope.status)}`} />
...
// 					<div className="min-w-0 flex-1">
// 						<h3 className="text-foreground mb-1 truncate text-sm font-medium">{envelope.title}</h3>
...
// 					<div className={`h-2 w-2 rounded-full ${getStatusColor(envelope.status)}`} />
// 					<span className="text-muted-foreground text-xs capitalize">{envelope.status}</span>
// 					<Badge
// 						variant="outline"
...
// 												<Progress value={progress.percent} className="h-1 w-16" />
...
// 							<span className="text-muted-foreground text-xs font-medium">Your Status</span>
// 						</div>
...
// 											</AvatarFallback>
// 										</Avatar>
// 										<span className="text-foreground text-xs">{session?.user?.name}</span>
// 									</div>
