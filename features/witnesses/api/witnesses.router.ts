...
export const witnessesRouter = createTRPCRouter({
	// Create new witness
	createWitness: protectedProcedure
		.input(createWitnessSchema)
	...
	getMyWitnesses: protectedProcedure
		.input(getWitnessesSchema)
			...
			try {
				...
				if (error instanceof Error && error.message.includes("does not exist")) {
					throw new TRPCError({
						...
						message: "Witness table does not exist. Please run database migrations: npm run db:generate && npm run db:push",
					})
				}
				throw error
			}
	...
	// Get witness by ID
	getWitnessById: protectedProcedure
		.input(getWitnessByIdSchema)
		.query(async ({ ctx, input }) => {
	...
	// Verify witness ID
	verifyWitness: protectedProcedure
		.input(verifyWitnessSchema)
		.mutation(async ({ ctx, input }) => {
			const userId = ctx.session.user.id
