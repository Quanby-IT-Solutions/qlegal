		...
		.mutation(async ({ ctx, input }) => {
			...
			await ctx.db.user.create({
				data: {
					...
					role: "CLIENT" as const // Default role for all new registrations
				}
			})
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!user?.recoveryEmail) {
				return {
					success: true,
					message:
						"If a recovery email exists, a password reset code has been sent"
				}
			}
			...
			try {
				...
				console.error(
					"Failed to send password reset code to recovery email:",
					error
				)
				throw new TRPCError({
					code: "INTERNAL_SERVER_ERROR",
