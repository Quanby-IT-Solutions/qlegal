...
export function UnifiedSignatureDialog({
  ...
  const formSchema = z.object({
    value: currentField
      ? createFieldSchema(currentField.type, currentField.required)
      : z.string()
  })
          ...
          <div className="space-y-4">
            <SignaturePad
              ...
              onSignatureChange={(hasSignature) => {
                ...
                if (signaturePadRef.current) {
                  const signatureType =
                    signaturePadRef.current.getSignatureType()
                  const isEmpty = signaturePadRef.current.isEmpty()
                ...
                if (hasSignature && signaturePadRef.current) {
                  ...
                  const signatureText =
                    signaturePadRef.current.getSignatureText()
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    if (currentField.type === "SIGNATURE" && currentField.required) {
      ...
      if (!isSignatureValid) {
        toast.error(
          "Please provide a signature using any method (draw, type, upload, or default)"
        )
        return
      }
      ...
      if (signaturePadRef.current) {
        ...
        if (!signatureType) {
          toast.error(
            "Please activate your signature by clicking the appropriate button"
          )
          return
        }
      }
    ...
    if (currentField.type === "SIGNATURE" && signaturePadRef.current) {
      ...
      if (!finalValue || finalValue.trim() === "") {
        toast.error(
          "No signature data available. Please create a signature first."
        )
        return
      }
  ...
  const hasValidSignature =
    currentField?.type === "SIGNATURE" ? isSignatureValid : !!formValue
      ...
      <DialogContent className="max-h-[95vh] w-[95vw] max-w-2xl gap-4 overflow-hidden p-4 sm:p-6">
        <DialogHeader className="pb-4 sm:pb-6">
          <div className="mb-2 flex items-center gap-2 sm:gap-3">
            <div className="rounded-lg bg-gradient-to-r from-primary/80 to-primary p-1.5 sm:p-2">
            ...
            <Form {...form}>
              ...
              >
                ...
                <Card className="border-2 border-primary/20 shadow-lg">
                  <CardHeader className="bg-gradient-to-r from-primary/5 to-primary/10 pb-3 sm:pb-4">
                                ...
                                <div className="rounded-md border border-amber-200 bg-amber-50 p-3">
                                  <div className="flex items-center gap-2">
                                    ...
                                    <p className="text-sm font-medium text-amber-800">
                                      Please provide a signature using any
                                      method
                                    </p>
                                  </div>
                                  <p className="mt-1 text-xs text-amber-700">
                                    You can draw, type, upload an image, or use
                  ...
                  <CardContent className="pt-4 sm:pt-6">
                    <div className="grid grid-cols-1 gap-3 text-sm sm:grid-cols-2 sm:gap-4">
                      ...
                      <div>
                        ...
                        <span className="ml-2">
                          {currentField.position.pageNumber}
                        </span>
                      </div>
                      <div>
                        <span className="font-medium text-foreground">
                ...
                <div className="flex flex-col-reverse gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between sm:gap-0 sm:pt-6">
                  ...
                  <Button
                    ...
                    className={`w-full transition-all duration-200 sm:w-auto ${canSubmit && !signFieldMutation.isPending
