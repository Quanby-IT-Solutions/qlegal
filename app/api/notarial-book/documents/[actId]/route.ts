...
export async function GET(
	...
	try {
		...
		if (!act) {
			return new NextResponse("Notarial act not found.", {
				status: 404,
				headers: {
					"Content-Type": "text/plain",
				},
		...
		if (projectUuid) {
			try {
				...
				console.warn(
					"⚠️ Status check failed but continuing (might be temporary API issue):",
					statusErrorMessage
				)
			}
		}
			...
			if (act.documentId) {
				...
				if (document?.path) {
					...
					if (fileData && !downloadError) {
						...
						console.log(
							"✅ Successfully fetched PDF from Supabase storage, size:",
							buffer.length,
							"bytes"
						)
					...
					if (downloadError) {
						...
						if (cleanPath !== document.path.trim()) {
							...
							if (fileData2 && !downloadError2) {
								...
								console.log(
									"✅ Successfully fetched PDF from Supabase with cleaned path, size:",
									buffer.length,
									"bytes"
								)
				...
				try {
					...
					if (fileResponse.ok) {
						...
						if (
							...
							console.warn(
								"⚠️ Vault file URL did not return a PDF (Content-Type:",
								contentType,
								"), trying downloadSignedDocument..."
							)
						}
					} else {
						console.warn(
							"⚠️ Vault file URL returned status:",
							fileResponse.status,
					...
					console.warn(
						"⚠️ Failed to fetch PDF from Vault file URL:",
						fetchError,
						", trying downloadSignedDocument..."
					)
				}
		...
		try {
			...
			console.log(
				"✅ Successfully downloaded signed document from DocoChain, size:",
				buffer.length,
				"bytes"
			)
			...
			if (
				...
				console.warn(
					"⚠️ Document is not fully signed - refusing to serve unsigned document from Supabase"
				)
				return new NextResponse(
			...
			if (isTemporaryError && act.documentId) {
				console.warn(
					"⚠️ DocoChain temporary error - trying Supabase fallback (NOTE: may be unsigned original document)"
				)
				try {
					const document = await db.query.documents.findFirst({
						where: eq(documents.id, act.documentId),
					...
					if (document?.path) {
						...
						if (fileData && !supabaseError) {
							...
							console.warn(
								"⚠️ Serving document from Supabase storage - NOTE: This may be the original unsigned document, not the signed version"
							)
						...
						if (supabaseError) {
							...
							if (cleanPath !== document.path.trim()) {
								...
								if (fileData2 && !supabaseError2) {
									...
									console.warn(
										"⚠️ Serving document from Supabase with cleaned path - NOTE: This may be the original unsigned document"
									)
