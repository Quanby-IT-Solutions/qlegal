...
function DataGridTableViewport({
	...
	useEffect(() => {
		...
		const scrollViewport =
			viewportElement.closest('[data-slot="scroll-area-viewport"]') ?? viewportElement.parentElement
...
function DataGridTableRenderedRow<TData>({
	...
	const { props } = useDataGrid()
	const showExpandedRow = props.expandRowBy?.(row.original) ?? row.getIsExpanded()
	...
	return (
		<Fragment>
			...
			<AnimatePresence initial={false} mode="sync">
				{showExpandedRow ? <DataGridTableBodyRowExpandded key={row.id} row={row} /> : null}
			</AnimatePresence>
		</Fragment>
	)
}
...
function DataGridTableRowSelect<TData>({ row }: { row: Row<TData> }) {
	return (
		<>
			<div
				className={cn(
					"bg-primary inset-s-0 absolute top-0 bottom-0 hidden w-0.5",
