...
function useHighlight(): HighlightContextType<string> {
...
const Highlight = React.forwardRef<HTMLDivElement, HighlightProps<React.ElementType>>(
...
function isExtendedChildElement(
	node: React.ReactNode
): node is React.ReactElement<ExtendedChildProps> {
...
const HighlightItem = React.forwardRef<HTMLDivElement, HighlightItemProps>(function HighlightItem(
	{
		as,
		children,
		id,
	...
	const isValidElement = isExtendedChildElement(children)
	const element: React.ReactElement<ExtendedChildProps> | null = isValidElement ? children : null
	const childValue = id ?? value ?? element?.props?.["data-value"] ?? element?.props?.id ?? itemId
	const isActive = activeValue === childValue
	const isDisabled = Boolean(disabled ?? contextDisabled ?? false)
	const itemTransition: Transition | undefined = transition ?? contextTransition
	...
	const commonHandlers: React.HTMLAttributes<HTMLDivElement> = hover
		? {
		...
		if (mode === "children") {
			return React.cloneElement(
				element,
				{
					...
					}) as Partial<ExtendedChildProps>),
					...commonHandlers,
					...props,
				} as Partial<ExtendedChildProps> & React.Attributes,
				<>
					<AnimatePresence initial={false} mode="wait">
						{isActive && !isDisabled && (
							<motion.div
