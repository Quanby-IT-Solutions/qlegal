...
function GridBackground({
	...
	// Generate beam configurations - using deterministic values to avoid hydration issues
				...
				return {
					...
					gridLine:
						direction === "horizontal"
							? (i % (rows ?? 0 - 1)) + 1 || 1
							: (i % (cols ?? 0 - 1)) + 1 || 1,
					delay: (i * 0.3) % 2, // Deterministic delay based on index
					duration: speed + ((i * 0.2) % 2), // Deterministic duration based on index
				}
			}),
	...
	return (
		...
		>
			...
			>
				{/* Grid Cells */}
				{Array.from({ length: (cols ?? 0) * (rows ?? 0) }).map((_, index) => (
					<div
						key={index}
			...
			{animatedBeams.map(beam => {
				...
				const horizontalPosition = (beam.gridLine / (rows ?? 0)) * 100
				const verticalPosition = (beam.gridLine / (cols ?? 0)) * 100

				return (
					<motion.div
						key={beam.id}
						className={cn(
							"absolute z-0 rounded-full backdrop-blur-sm",
							beam.color,
